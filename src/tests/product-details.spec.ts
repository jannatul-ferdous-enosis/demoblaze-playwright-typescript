...
test.describe('Product Detail Page', () => {
  ...
  test('4.2 Product image loads in detail carousel', async () => {
    ...
    await productDetailPage.productImage.waitFor({ state: 'visible', timeout: 15000 });
  ...
  test('4.7 Navigate to invalid product ID', async ({ page }) => {
    ...
    const pageDidNotCrash = await page.evaluate(() => !document.querySelector('.error, .alert-danger'));
    expect(pageDidNotCrash).toBe(true);
  });

  test('4.8 Navigate to product page without ID parameter', async ({ page }) => {
    await productDetailPage.gotoDirect();
    await page.waitForTimeout(2000);

    const nameVisible = await productDetailPage.productName.isVisible().catch(() => false);
  ...
  test('4.10 Rapid double-click "Add to cart"', async () => {
    ...
    const dialogPromise = productDetailPage.page.waitForEvent('dialog', { timeout: 10_000 });
